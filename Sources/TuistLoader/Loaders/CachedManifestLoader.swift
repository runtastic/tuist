...
public class CachedManifestLoader: ManifestLoading {
    ...
    public func loadProject(at path: AbsolutePath, rootPath: AbsolutePath? = nil) async throws -> Project {
        try await load(manifest: .project, at: path) {
            try await manifestLoader.loadProject(at: path, rootPath: rootPath)
        }
    }
